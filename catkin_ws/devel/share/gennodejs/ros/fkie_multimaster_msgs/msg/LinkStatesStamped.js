...
class LinkStatesStamped {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.links.forEach((val) => {
      bufferOffset = LinkState.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new LinkStatesStamped(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.links[i] = LinkState.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.links.forEach((val) => {
      length += LinkState.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new LinkStatesStamped(null);
    ...
    if (msg.links !== undefined) {
      ...
      for (let i = 0; i < resolved.links.length; ++i) {
        resolved.links[i] = LinkState.Resolve(msg.links[i]);
      }
    }
    else {
      resolved.links = []
...
module.exports = LinkStatesStamped;
